...
class SquatCounter(ExerciseBase):
    ...
    def __init__(
        ...
        hip_drop_min: float = 0.18,
        start_drop: float = 0.08,
    ):
        self.rep_count = 0
        self.phase = "TOP"
        self._thresholds = SquatThresholds(
    ...
    def update(self, pose: PoseFrame, bar_y: Optional[float] = None) -> ExerciseState:
        ...
        if self.phase == "TOP":
            ...
            if knee_angle >= self._thresholds.top_knee_angle - 5 and hip_drop <= 0.04:
                self.rep_count += 1
                self.phase = "TOP"
...
def _select_side(lm) -> Optional[Tuple[Landmark2D, Landmark2D, Landmark2D]]:
    ...
    if left_valid and right_valid:
        return left  # prefer left for consistency
    if left_valid:
        return left
    if right_valid:
        return right
